...
public class Vaga implements Entidade {
	...
	@DBRef
	private Map<String, Candidato> candidatosSelecionados = Maps.newHashMap();
	...
	public Map<String, Candidato> getCandidatosSelecionados() {
		return candidatosSelecionados;
	}

	public void setCandidatosSelecionados(Map<String, Candidato> candidatosSelecionados) {
	...
	public void selecionarCandidatos(List<Candidato> todosOsCandidatos) {
		...
		if (candidatosSelecionados == null) {
			this.candidatosSelecionados = Maps.newHashMap();
		}
		...
		for (Candidato candidato : todosOsCandidatos) {
			...
			if (candidato.estaAptoComecarData(this.periodo.getDataInicial())) {
				...
				if (pontuacao != 0) {
					candidatosSelecionados.put(String.valueOf(pontuacao), candidato);
				}
			}
	...
	public void iniarEntrevistas() throws BusinessException {
		...
		if (!comStatus(SELECIONANDO_CANDIDATOS)) {
			throw new BusinessException(MUDANCA_NAO_PERMITIDA);
		}

		if (!hasCandidatos()) {
			throw new BusinessException(VAGA_SEM_CANDIDATOS_SELECIONADOS);
		}
